...
public abstract class AbstractHttpClient {
  ...
  public <T> Result<T> post(final Request request, final ParameterizedTypeReference<Result<T>> dataClass) {
    ...
    ResponseEntity<Result<T>> responseEntity = restTemplate
        .exchange(this.baseUrl().concat(request.getPath()), HttpMethod.POST, new HttpEntity<>(request, this.buildHeaders(request)), dataClass);
  ...
  public <T> Result<T> put(final Request request, final ParameterizedTypeReference<Result<T>> dataClass) {
    ...
    ResponseEntity<Result<T>> responseEntity = restTemplate
        .exchange(this.baseUrl().concat(request.getPath()), HttpMethod.PUT, new HttpEntity<>(request, this.buildHeaders(request)), dataClass);
    this.logResponse(responseEntity.getBody(), request.getPath());
    return responseEntity.getBody();
  }
